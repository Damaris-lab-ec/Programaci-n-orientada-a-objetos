...
class Inventario:
    ...
    def eliminar_producto(self, id_producto):
        self.__productos = [p for p in self.__productos if p.get_id_producto() != id_producto]
        self.guardar_en_archivo()
    ...
    def buscar_por_nombre(self, nombre):
        encontrados = [p for p in self.__productos if nombre.lower() in p.get_nombre().lower()]
...
def menu():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            print(resultados if isinstance(resultados, str) else "\n".join(map(str, resultados)))
        elif opcion == "5":
